...
// @MX:NOTE [AUTO] Execution tests for POST /api/validation/iq (SPEC-REGULA-VALIDATION-001).
// @MX:SPEC SPEC-REGULA-VALIDATION-001 (M1, REQ-VAL-003, Issue #49)
//
// No prior test existed (0% coverage). Spawns scripts/validation/collect-iq.ts;
// node:child_process.spawn is mocked with a controllable child emitter covering
// success / non-zero-exit / spawn-error / json-parse / validation branches.
...
const { POST } = await import('@/app/api/validation/iq/route');

function postReq(body: unknown): Request {
  return new Request('http://localhost/api/validation/iq', {
    method: 'POST',
...
describe('POST /api/validation/iq (REQ-VAL-003)', () => {
  it('returns 200 with the collector JSON summary on success', async () => {
    const summary = { releaseId: 'v1.2.3', evidenceCount: 12 };
  ...
  it('returns 500 IQ bundle collection failed on a non-zero exit code', async () => {
    spawn.mockImplementationOnce(() => makeChild({ stdout: 'boom', exitCode: 2 }));
    const res = await POST(postReq({ releaseId: 'v1.2.3' }), {});
    expect(res.status).toBe(500);
    const body = await res.json();
    expect(body.error).toBe('IQ bundle collection failed');
