...
function buildPdfDocument(comparison: SavedComparison): React.ReactElement {
  ...
  const cellViews = comparison.cells.map((cell, i) => {
    const predLines = cell.predicate_texts.map((t, j) =>
      e(Text, { key: `pt-${j}` }, `Predicate ${j + 1}: ${String(t)}`),
    );
    return e(
      View,
      { key: `cell-${i}`, style: styles.cell, wrap: false },
      ...[
        e(Text, { style: styles.dimension }, String(DIMENSION_LABELS[cell.dimension] ?? cell.dimension)),
        e(Text, {}, `Subject: ${String(cell.subject_text)}`),
        ...predLines,
      ],
    );
  });
  ...
  const pageChildren = [
    e(View, { key: 'disclaimer', style: styles.disclaimer },
      e(Text, { style: styles.disclaimerHeading }, 'Regulatory Disclaimer'),
      e(Text, {}, DISCLAIMER),
    ),
    e(Text, { key: 'title', style: styles.title },
      `Predicate Device Comparison — ${String(comparison.subject_device_name)}`,
    ),
    ...predicateLines,
    e(View, { key: 'cells', style: { marginTop: 12 } }, ...cellViews),
    e(Text, { key: 'footer', style: styles.footer, fixed: true },
      'Generated by Regula — For RA professional review only',
    ),
  ];

  return e(Document, {}, e(Page, { size: 'A4', style: styles.page }, ...pageChildren));
}
