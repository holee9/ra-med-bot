...
describe('POST /api/webhooks/audit', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('should return 401 when X-Regula-API-Key header is missing', async () => {
    ...
    const req = new Request('http://localhost:3000/api/webhooks/audit', {
      method: 'POST',
      body: JSON.stringify({ tenant_id: 'test', event_type: 'test', product_id: 'test', data: {} }),
    });
