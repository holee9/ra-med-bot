...
export const chunkSopIso13485: ChunkerFn = (text, _metadata): Chunk[] => {
  ...
  const bodyStartIdx = lines.findIndex(
    (l, i) => i > (revHistoryIdx > -1 ? revHistoryIdx + 2 : 0) && /^\d+[\.\s]/.test(l.trim()),
  );
  ...
  if (headerText) {
    chunks.push({
      ...
      metadata: {
        ...
        sectionPath: 'Header',
        tokenCount: countTokens(headerText),
      },
    });
  }
  ...
  if (revHistoryIdx > -1) {
    ...
    if (revText) {
      chunks.push({
        ...
        metadata: {
          docClass: DocClass.internal_sop,
          sectionPath: 'Revision History',
          tokenCount: countTokens(revText),
        },
      });
    }
  }
