...
// Kernel tables + enums moved to schema-kernel.ts (SPEC-V3-RESTRUCTURE-001 Phase B).
// Re-exported here for backward compatibility — `@/lib/db/schema` imports still resolve.
import {
  localeEnum,
  sessions,
  themePrefEnum,
...
// ---------------------------------------------------------------------------
// localeEnum + themePrefEnum moved to schema-kernel.ts (kernel-only, used by users table).
...
// userRoleEnum + userStatusEnum + userDepartmentEnum moved to schema-kernel.ts (kernel-only).

// @MX:NOTE audit_action values mirror AuditAction type in lib/kernel/audit.ts.
...
// `users` table moved to schema-kernel.ts (kernel boundary — SPEC-V3-RESTRUCTURE-001 B2).
// `users` is imported from './schema-kernel' at the top of this file and re-exported.
...
// @MX:NOTE: [AUTO] review_required is enforced server-side; see lib/kernel/auth/with-workflow-review.ts
...
// ---------------------------------------------------------------------------
// Auth.js v5 DrizzleAdapter tables — sessions + verificationTokens moved to
// schema-kernel.ts (kernel auth layer). `accounts` remains here (OAuth provider
// data — domain-level, not kernel). Migration: 0023_auth_adapter_tables.sql
// ---------------------------------------------------------------------------
// sessions + verificationTokens: see schema-kernel.ts (imported + re-exported above).
...
// verificationTokens moved to schema-kernel.ts (imported + re-exported above).
...
// @deprecated 아카이브됨 — archive/qms-pms/ 참조, Phase A #530 (SPEC-V3-RESTRUCTURE-001 B7)
