...
> **범위 한정 (2026-06-28 개정)**: 본 제안은 **축소(Step 1~4)** 로 완결. 계층 2/3 모듈화(§6.4)·CER/PCCP 사이드바 진입점·실사용 로그 분석·pre-existing lint 정리 등은 **본 제안 범위 외** (별개 결정 사항). "후속 권장"으로 확장 제안을 덧붙이지 않는다 — 방향 이탈 방지.
...
## 8. 실행 로드맵 — 완료 (2026-06-28 ~ 2026-06-29)

> **최종 상태 (2026-06-29)**: 본 제안의 축소(Step 1~4)는 완결. 문서 개정(Issue #306)으로 **정체성 최종 확정** 반영.

| 단계 | 내용 | 상태 | 산출물 |
|---|---|---|---|
| **Step 0** | 제안서 검토 + 사용자 승인 | ✅ 완료 | 본 문서 (채택됨) |
| **Step 1** | feature flag 레지스트리 도입 (계층 1) | ✅ 완료 (PR #305) | `lib/feature-flags.ts` 8종 |
| **Step 2** | RETIRE/FREEZE UI 숨김 (flag off) | ✅ 완료 (PR #305) | 사이드바 8개 링크 숨김 |
| **Step 3** | Charter 정정 문서화 (지양-3/5) | ✅ 완료 (PR #305) | `docs/scope-boundary.md` + README |
| **Step 4** | 킬러 4종 + 외부의존 4종 구현 중단 공식화 | ✅ 완료 | 이슈 8종 close-out 코멘트 |
| **Step 5** | **정체성 최종 확정 문서 반영** (2026-06-29) | ✅ 완료 (Issue #306) | README.md + docs/scope-boundary.md + 본 문서 |

**본 제안 범위 외 (별개 결정, 본 제안에서 실행하지 않음)**:
- ~~계층 2/3 모듈화~~ — §6.4 참고용 검토 결과일 뿐, 본 제안(축소)과 무관. 향후 별개 검토.
- ~~실사용 로그 분석 → KEEP/SHRINK 경계 확정~~ — 별개 세션.
- ~~CER/PCCP 사이드바 진입점 추가~~ — 본 제안(축소)과 **반대 방향**(확장). 별개.
- ~~pre-existing lint warnings 정리~~ — 본 작업 무관.
