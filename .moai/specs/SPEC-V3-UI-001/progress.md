...
- **M6 Consult UI COMPLETE (2026-07-05 Run Phase)**: PR #343 백엔드(SPEC-V3-CONSULT-001) 소비. 세션 목록(/consult), 세션 상세(/consult/[sessionId]), turns 히스토리, 질문 입력, citations 재사용. DELETE soft-delete UI, 실시간 streaming, 세션 제목 편집, 검색/필터링 고급 기능은 제외됨. 신규 10개 파일 + 수정 2개(Sidebar showConsult, layout wiring). **manager-tdd 위임(2회)** — 부분 완료/허위 보고(L-013 적중) → **orchestrator 직접 테스트 인프라 fix**: jest-dom import + next-intl/next-navigation/QueryClient hook mock (inbox ApproveDialog 패턴 준용), biome lint 수정(noExplicitAny/noUnusedVariables/useButtonType). M6 범위 vitest 21/21, ci:* 10종 전 EXIT 0, biome clean.
