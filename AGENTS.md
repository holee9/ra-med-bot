...
Core principles (1-4) and six Agent Core Behaviors (consolidated cross-cutting rules) are defined in .Codex/rules/moai/core/moai-constitution.md. Development safeguards (5-9) are detailed in Section 7.
...
Definition: Hybrid design workflow — Codex Design (path A) or code-based brand design (path B).

Subcommands: design (unified entry point)
Default (natural language): Routes to /moai design with AskUserQuestion path selection (Codex Design vs code-based)

For detailed design rules, see .Codex/rules/moai/design/constitution.md
...
Migration guide: see .Codex/commands/agency/agency.md
...
For detailed agent descriptions, see the Agent Catalog section above. For agent creation guidelines, use the builder-agent subagent or see `.Codex/rules/moai/development/agent-authoring.md`.
...
For detailed workflow specifications, see .Codex/rules/moai/workflow/spec-workflow.md
...
For MX protocol details, see .Codex/rules/moai/workflow/mx-tag-protocol.md

For team-based parallel execution of these phases, see .Codex/skills/moai/team/plan.md and .Codex/skills/moai/team/run.md.

---

## 6. Quality Gates

For TRUST 5 framework details, see .Codex/rules/moai/core/moai-constitution.md
...
Constraints:
- Maximum 4 questions per AskUserQuestion call (Codex limit)
...
MoAI-ADK uses Codex's official rules system at `.Codex/rules/moai/`:
...
- **Design rules**: Design system constitution (.Codex/rules/moai/design/constitution.md)
...
- `.Codex/rules/moai/design/constitution.md`: FROZEN/EVOLVABLE zone definitions, safety architecture
...
For anti-hallucination policy, see .Codex/rules/moai/core/moai-constitution.md
...
- **UltraThink** (`ultrathink` keyword): Sets `effort: max` in Codex v2.1.110+. For Codex-opus-4-7, this triggers Adaptive Thinking (dynamically allocated reasoning tokens, no fixed budget_tokens). For older models, maps to extended thinking with high budget. No MCP dependency — compatible with all APIs. Do NOT confuse with `--deepthink`.
- **Adaptive Thinking** (Codex-opus-4-7 only): Opus 4.7's thinking mode. Unlike earlier models that use `budget_tokens`, Adaptive Thinking dynamically allocates reasoning based on task complexity. Triggered via `effort` level (high/xhigh/max) — not by `budget_tokens`. See Skill("moai-workflow-thinking").
...
- **Codex-in-chrome**: Browser automation for web-based tasks.

For MCP configuration and usage patterns, see .Codex/rules/moai/core/settings-management.md.
...
**Level 3** (Bundled): On-demand, Codex decides when to access
...
For core parallel execution principles, see .Codex/rules/moai/core/moai-constitution.md.
...
For the complete worktree selection decision tree, see .Codex/rules/moai/workflow/worktree-integration.md
...
For complete Agent Teams documentation including team API reference, role profiles, file ownership strategy, team workflows, and configuration, see .Codex/rules/moai/workflow/spec-workflow.md and .moai/config/sections/workflow.yaml.

### CG Mode (Codex + GLM Cost Optimization)

MoAI-ADK supports CG Mode for 60-70% cost reduction on implementation-heavy tasks via tmux Agent Teams:

```
┌─────────────────────────────────────────────────────────────┐
│  LEADER (Codex, current tmux pane)                         │
...
- Security reviews (needs Codex's security training)
...
MoAI searches previous Codex sessions when context is needed to continue work on existing tasks or discussions.
...
3. Use Grep to search session index and transcript files in ~/.Codex/projects/
...
When MoAI workflows behave unexpectedly, use Codex's built-in debug tools:
