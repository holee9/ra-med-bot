...
async function evaluateGoNoGo(page, criteria: any, personaName: string) {
  // Navigate to evaluation page
  await page.goto(`/evaluation/${personaName}`);

  // Fill evaluation form
  for (const [category, requirements] of Object.entries(criteria)) {
    if (typeof requirements === 'object' && !requirements.passingScore) {
      for (const [key, value] of Object.entries(requirements as any)) {
  ...
  const score = parseFloat(await page.textContent('[data-testid="evaluation-score"]'));
...
test.describe('RA Lead Go/No-Go Evaluation', () => {
  ...
  test('RA Lead Go/No-Go 기준 충족', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="eu_mdr-experience"]')).toContainText('Class IIb/III 제출 경험 2건 이상');
    await expect(page.locator('[data-testid="regulation-knowledge"]')).toContainText('FDA/EU MDR/MFDS/NMPA/PMDA 규제 지식');
    await expect(page.locator('[data-testid="documentation-knowledge"]')).toContainText('영어 문서 작성 능력');
  });
...
test.describe('Clinical Lead Go/No-Go Evaluation', () => {
  ...
  test('Clinical Lead Go/No-Go 기준 충족', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="medical_license-certification"]')).toContainText('의사 면허 필수');
  });
...
test.describe('R&D Lead Go/No-Go Evaluation', () => {
  ...
  test('R&D Lead Go/No-Go 기준 충족', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="engineering-knowledge"]')).toContainText('공학적 지식 (기계/전공/소프트웨어)');
  });
});
...
test.describe('Engineering Specialist Go/No-Go Evaluation', () => {
  ...
  test('Engineering Specialist Go/No-Go 기준 충족', async ({ page }) => {
    const evaluation = await evaluateGoNoGo(page, ENGINEERING_SPECIALIST_GO_CRITERIA, 'engineering-specialist');

    expect(evaluation.decision).toBe('GO');
    expect(evaluation.score).toBeGreaterThanOrEqual(ENGINEERING_SPECIALIST_GO_CRITERIA.passingScore);
    expect(evaluation.passed).toBe(true);
  });
});
...
test.describe('QA Lead Go/No-Go Evaluation', () => {
  ...
  test('QA Lead Go/No-Go 기준 충족', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="qm_license-certification"]')).toContainText('품질경리(QM) 자격증 보유');
  });
});
...
test.describe('Documentation Specialist Go/No-Go Evaluation', () => {
  ...
  test('Documentation Specialist Go/No-Go 기준 충족', async ({ page }) => {
    const evaluation = await evaluateGoNoGo(page, DOCUMENTATION_SPECIALIST_GO_CRITERIA, 'documentation-specialist');

    expect(evaluation.decision).toBe('GO');
    expect(evaluation.score).toBeGreaterThanOrEqual(DOCUMENTATION_SPECIALIST_GO_CRITERIA.passingScore);
    expect(evaluation.passed).toBe(true);
  });
});
...
test.describe('Legal Counsel Go/No-Go Evaluation', () => {
  ...
  test('Legal Counsel Go/No-Go 기준 충족', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="expert_review-knowledge"]')).toContainText('Expert Review Gate 승인 권한');
  });
});
...
test.describe('종합 Go/No-Go 결정 프로세스', () => {
  ...
  test('모든 페르소나 Go/No-Go 평가 및 종합 결정', async ({ page }) => {
    ...
    const overallScore = parseFloat(await page.textContent('[data-testid="overall-score"]'));
    ...
    await expect(page.locator('[data-testid="audit-entry"]')).toContainText('Overall Go/No-Go decision: GO');
