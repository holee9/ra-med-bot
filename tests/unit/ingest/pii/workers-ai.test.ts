...
describe('detectPiiWorkersAi', () => {
  ...
  afterEach(() => {
    Reflect.deleteProperty(process.env, 'CF_WORKERS_AI_TOKEN');
    Reflect.deleteProperty(process.env, 'CF_ACCOUNT_ID');
  });
  ...
  it('returns array of PIISpan objects', async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn().mockResolvedValue({
        ...
        json: () =>
          Promise.resolve({
            result: [{ entity: 'PERSON', score: 0.95, start: 8, end: 16, word: 'John Doe' }],
          }),
      }),
    );
