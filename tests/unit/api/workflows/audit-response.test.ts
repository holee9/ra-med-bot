...
const validBody = {
  ...
  input_format: 'text',
  input_content:
    'During the inspection conducted at your facility on January 15, 2026, the following observations were noted regarding your quality management system and corrective action procedures. The FDA investigator identified significant deficiencies in your CAPA process.',
...
describe('POST /api/ra/workflows/audit-response', () => {
  ...
  it('returns 400 for input_content too short (< 100 chars)', async () => {
    const body = { ...validBody, input_content: 'Too short content' };
...
describe('GET /api/ra/workflows/audit-response/[runId]/status', () => {
  it('returns 200 with status payload for a valid UUID runId', async () => {
    const req = new Request(
      `http://localhost/api/ra/workflows/audit-response/${VALID_UUID}/status`,
    );
