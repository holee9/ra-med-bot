...
describe('lib/db/schema.ts Phase 5 additions', () => {
  ...
  it('auditActionEnum stays in lock-step with AuditAction through risk management', () => {
    ...
    expect(values).toHaveLength(228); // +4 workflow.* (SPEC-REGULA-WORKFLOWS-LLM-002 M0, REQ-WFLLM-007/008); was 224
    expect(typeValues).toHaveLength(228);
  });
...
describe('SPEC-REGULA-PMS-001 (Issue #53) — migration 0069', () => {
  ...
  // SPEC-REGULA-PHI-REMOVAL-001 (Issue #319): the 3 PMS workflow_type values
  // were REMOVED from schema.ts workflowTypeEnum by migration 0103. This
  ...
  it('schema.ts workflowTypeEnum NO LONGER includes the 3 PMS values (Issue #319)', () => {
    ...
    expect(enumBlock).not.toContain("'pms_report'");
    expect(enumBlock).not.toContain("'pmcf_plan'");
    expect(enumBlock).not.toContain("'pmcf_evaluation'");
  });
  ...
  it('schema.ts auditActionEnum NO LONGER includes the PMS values (Issue #319)', () => {
    ...
    for (const v of PMS_AUDIT_ACTIONS) {
      expect(enumBlock).not.toContain(`'${v}'`);
    }
  });
  ...
  it('AuditAction type in audit.ts NO LONGER includes the PMS values (Issue #319)', () => {
    ...
    for (const v of PMS_AUDIT_ACTIONS) {
      expect(typeBlock).not.toContain(`'${v}'`);
    }
  });
});
