...
describe('WorkflowStatusSchema', () => {
  it('accepts all 7 valid statuses', () => {
    const statuses = ['queued', 'running', 'paused', 'pending_review', 'approved', 'rejected', 'failed'];
...
describe('SubmissionDrafterInputSchema', () => {
  ...
  it('rejects short indications_for_use', () => {
    expect(() => SubmissionDrafterInputSchema.parse({ ...valid, indications_for_use: 'short' })).toThrow();
  });
  it('validates K-number format in predicate_k_numbers', () => {
    expect(() => SubmissionDrafterInputSchema.parse({ ...valid, predicate_k_numbers: ['INVALID'] })).toThrow();
    expect(SubmissionDrafterInputSchema.parse({ ...valid, predicate_k_numbers: ['K123456'] })).toBeDefined();
  });
  it('rejects more than 3 predicate K-numbers', () => {
    expect(() => SubmissionDrafterInputSchema.parse({
      ...valid,
...
describe('IndicationImpactInputSchema', () => {
  ...
  it('rejects more than 5 target_markets', () => {
    expect(() => IndicationImpactInputSchema.parse({
      ...valid,
